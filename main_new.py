...
#### turn long clause as OR gate
LONG_CLAUSE_OR = False
CLAUSE_MAX_LENGTH = 6

#### keep long clause for hybrid format
HYBRID_LONG_CLAUSE = False

#### minimum length of long clause
LONG_CLAUSE_MIN_LENGTH = 10

BCP_BEFORE_PACKING = True
...
def bcp(cnf, no_vars):
    ...
    # 建立变量到子句的映射
    var_to_clauses = {}
    for i in range(-no_vars, no_vars + 1):
        if i != 0:
            var_to_clauses[i] = []
    ...
    for clause_idx, clause in enumerate(cnf):
        for lit in clause:
            var_to_clauses[lit].append(clause_idx)
    
    # 记录每个变量的赋值(1表示True, -1表示False, 0表示未赋值)
    assignments = [0] * (no_vars + 1)
    ...
    def propagate(lit):
        ...
        if assignments[var] != 0:
            return True if assignments[var] == value else False
        
        assignments[var] = value
        decided_clauses.append([lit])
        
        # 遍历包含var或-var的所有子句
        clauses_to_check = set(var_to_clauses[lit] + var_to_clauses[-lit])
        for clause_idx in clauses_to_check:
            if new_cnf[clause_idx] is None:
                continue
                
            clause = new_cnf[clause_idx]
            new_clause = []
            satisfied = False
            ...
            for l in clause:
                ...
                if assignments[v] == 0:
                    new_clause.append(l)
                elif (assignments[v] == 1 and l > 0) or (assignments[v] == -1 and l < 0):
                    satisfied = True
                    break
            
            if satisfied:
                new_cnf[clause_idx] = None
            else:
                if len(new_clause) == 0:
                    return False  # 发现空子句,UNSAT
                elif len(new_clause) == 1:
                    if not propagate(new_clause[0]):
                        return False
                new_cnf[clause_idx] = new_clause
                
        return True
    
    # 处理所有单子句
    for clause_idx, clause in enumerate(new_cnf):
        if clause is not None and len(clause) == 1:
            if not propagate(clause[0]):
                return None  # UNSAT
    
    # 返回简化后的CNF和已确定的变量
    result_cnf = [clause for clause in new_cnf if clause is not None]
    return result_cnf + decided_clauses
...
def divide_long_clauses(cnf, no_var, max_length=CLAUSE_MAX_LENGTH, hybrid=HYBRID_LONG_CLAUSE):
    res_cnf = []
    res_no_var = no_var
    long_clauses = []  
    
    assert not (LONG_CLAUSE_OR and HYBRID_LONG_CLAUSE), 'LONG_CLAUSE_OR and HYBRID_LONG_CLAUSE cannot be true at the same time'
    
    for clause in cnf:
        if len(clause) <= max_length:
            res_cnf.append(clause)             
        elif LONG_CLAUSE_OR and len(clause) > LONG_CLAUSE_MIN_LENGTH:
            long_clauses.append(clause)  
        elif hybrid and len(clause) > LONG_CLAUSE_MIN_LENGTH:
            long_clauses.append(clause)
        else:
...
def convert_cnf_xdata(cnf, no_vars,hybrid=HYBRID_LONG_CLAUSE):
    if BCP_BEFORE_PACKING:
        cnf = bcp(cnf, no_vars)
    if cnf == None:
    ...
    cnf, no_vars, long_clauses = divide_long_clauses(cnf, no_vars,hybrid=hybrid)
    ...
    return x_data, fanin_list, po_list, extra_pi, extra_po, long_clauses

def cnf2lut(cnf, no_vars, hybrid = HYBRID_LONG_CLAUSE):
    # Sort CNF
    no_clauses = len(cnf)
    # cnf = cnf_utils.sort_cnf(cnf)
    ...
    # p.print_stats()
    # exit(0)
    x_data, fanin_list, po_list, extra_pi, extra_po, long_clauses = convert_cnf_xdata(cnf, no_vars, hybrid)
    
    return x_data, fanin_list, po_list, extra_po, long_clauses
...
def main(cnf_path, output_bench_path):
    ...
    x_data, fanin_list, po_list, extra_po, long_clauses = cnf2lut(cnf, no_vars)
    print('convert time: {:.4f} s'.format(time.time() - convert_starttime))
    
    if x_data == None:
        print('CNF is UNSAT, no bench generated')
        return False
    ...
    # # Save long clauses
    # if HYBRID_LONG_CLAUSE:
    #     with open(output_bench_path.replace('.bench', '_long_clauses.cnf'), 'w') as f:
    #         for clause in long_clauses:
